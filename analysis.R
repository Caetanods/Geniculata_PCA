...
plot.phylo(tr, edge.width = 1.5, label.offset = 0.03, font = 4)
...
plot(1:length(male.var.prop), male.var.prop, type = "b", col = "blue"
   , ylim = c(0,1.0), xlim = c(0,length(male.var.prop))
   , xlab = "SPC axis", ylab = "Percent of variation"
   , main = "Dimensionality of the genitalia")
lines(1:length(female.var.prop), female.var.prop, type = "b", col = "red")
legend("topright", legend = c("Male","Female"), text.col = c("blue","red"), bty = "n")
...
comp.graphen <- geo.comp.rates(tr.grafen, cord.male, cord.female, plot = TRUE)
comp.unit <- geo.comp.rates(tr, cord.male, cord.female, plot = TRUE)
